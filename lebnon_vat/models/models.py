...
class lebnonvat(models.Model):
    ...
    def vjvwewe(self):
        for rec in self:
            rec.new_field=(rec.price_unit * rec.taxes_id.amount /100)
...
class saleorderlinelebnon(models.Model):
    ...
    def vjvwewe(self):
        for rec in self:
            rec.new_field=(rec.price_unit * rec.tax_id.amount /100)


    def subtotal_tax(self):
        for rec in self:
            if rec.price_unit:
                rec.new_sub_with_tax = (rec.price_unit * rec.tax_id.amount) + rec.price_unit  *(rec.dollar_2day)
...
class saleorderModule(models.Model):
    _inherit = 'sale.order'
    total_vat = fields.Monetary(string="بالليرة",  required=False,compute='shipping_one_r')
    untaxtotal_vat = fields.Monetary(string="باليرة",  required=False,compute='untaxtotal_amount')
    totallebnon_vat = fields.Monetary(string="Total",  required=False,compute='total_lebnon_vat')
